...
public class CopyOfPlayerCombat extends Action {
	...
	/*public int getMagicMaxHit(Player player, int baseDamage) {
		...
		if (target instanceof Player) {
			...
			def = p2.getSkills().getLevel(Skills.DEFENCE)
					+ (p2.getCombatDefinitions().getBonuses()[CombatDefinitions.MAGIC_DEF] * 2);
	...
	public int getRandomMaxHit(Player player, int weaponId, int attackStyle,
		...
		if (defenceAffects) {
			...
			double A = EA * (1 + (player.getCombatDefinitions().getBonuses()[ranging ? CombatDefinitions.RANGE_ATTACK
					: CombatDefinitions.getMeleeBonusStyle(weaponId,
							attackStyle)] / 64.0D));
