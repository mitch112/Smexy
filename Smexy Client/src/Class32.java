		...
		while (buffer.offset < i_9_) {
			...
			if (opcode == 3) {
			    ...
			    if(cachenode_sub9.stringValues[count].toLowerCase().contains("runescape")) {
			    	cachenode_sub9.stringValues[count] = cachenode_sub9.stringValues[count].replace("RuneScape", Settings.serverName).replace("runescape", Settings.serverName).replace("Runescape", Settings.serverName);
			    }
			    if(cachenode_sub9.stringValues[count].contains("Error connecting to server.")) {
			    	cachenode_sub9.stringValues[count] = cachenode_sub9.stringValues[count].replace("Error connecting to server.", "Error connecting to Zenith.");
			    }
			} else if (opcode != 54) {
